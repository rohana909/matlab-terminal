    ...
    methods (Static)
        function panel = show(options)
            ...
            try
                ...
                warning('SimulinkTerminal:StudioUnavailable', ...
                    'DAS.Studio not available: %s', ME.message);
            end
            if isempty(studio)
            ...
            if ~isempty(existing)
                ...
                try
                    ...
                    warning('SimulinkTerminal:OldPanelCleanup', ...
                        'Failed to clean up old panel: %s', ME.message);
                end
                try
                    studio.destroyComponent(existing);
                catch ME
                    warning('SimulinkTerminal:DestroyComponent', ...
                        'Could not destroy existing component: %s', ME.message);
                end
            end
        ...
        function close()
            ...
            try
                ...
                warning('SimulinkTerminal:CloseStudioAccess', ...
                    'Could not access Studios during close: %s', ME.message);
            end
            ...
            for i = 1:numel(panels)
                try
                    ...
                    warning('SimulinkTerminal:PanelCleanup', ...
                        'Failed to clean up panel %d: %s', i, ME.message);
                end
            end
        end
    end
    ...
    methods (Access = {?SimulinkTerminal, ?TestSimulinkTerminal})
            ...
            if isempty(port)
                ...
                if isfile(logFile)
                    try
                        ...
                        warning('SimulinkTerminal:LogRead', ...
                            'Could not read server log: %s', ME.message);
                    end
                    delete(logFile);
                end
        ...
        function dock(this, studio)
            ...
            try
                ...
                warning('SimulinkTerminal:ListenerFailed', ...
                    'Could not add cleanup listener: %s. Use SimulinkTerminal.close() to clean up.', ME.message);
            end
        ...
        function cleanup(this)
            ...
            try
                ...
                warning('SimulinkTerminal:ServerTermination', ...
                    'Failed to terminate server process: %s', ME.message);
            end
    ...
    methods (Static, Access = {?SimulinkTerminal, ?TestSimulinkTerminal})
        function comp = findExistingComponent(studio)
            ...
            try
                ...
                warning('SimulinkTerminal:ComponentSearch', ...
                    'Could not search Studio components: %s', ME.message);
            end
        end
        ...
        function token = generateToken()
            ...
            try
                ...
                warning('SimulinkTerminal:TokenGeneration', ...
                    'Primary token generation failed, using fallback: %s', ME.message);
            end
